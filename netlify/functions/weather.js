...
// Shared by concurrent invocations on the same warm instance so that when the
// cache is cold (e.g. right after several guests log in at once), only one
// upstream fetch happens instead of one per concurrent request.
let inflight = null;

async function fetchAndCache(now) {
...
exports.handler = async (event) => {
  ...
  try {
    if (!inflight) {
      inflight = fetchAndCache(now).finally(() => { inflight = null; });
    }
